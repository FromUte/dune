...
Dune.Users.Edit =
  ...
  init: Backbone.View.extend
    ...
    initialize: ->
      ...
      this.$('#user_channel_attributes_how_it_works, #user_channel_attributes_submit_your_project_text').markItUp(Dune.markdownSettings) if this.$('#user_channel_attributes_how_it_works').length > 0
      if window.File && window.FileReader && window.FileList && window.Blob
        this.prepareToDropzone()
        this.uploaders = []
        for uploader in $('.usr-upld-img')
          this.uploaders.push new Dune.Users.Edit.DragDropUploader { el: uploader }
