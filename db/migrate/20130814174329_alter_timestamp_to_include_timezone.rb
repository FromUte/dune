  ...
  def up
    execute "
      ...
      CREATE OR REPLACE FUNCTION expires_at(projects) RETURNS timestamptz AS $$
       SELECT (($1.online_date AT TIME ZONE coalesce((SELECT value FROM configurations WHERE name = 'timezone'), 'Europe/Paris') + ($1.online_days || ' days')::interval)::date::text || ' 23:59:59')::timestamptz
      $$ LANGUAGE SQL;
